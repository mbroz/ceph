...
bool PGLSParentFilter::filter(const std::string &obj_name,
...
bool PGLSPlainFilter::filter(const std::string &obj_name,
                             bufferlist& xattr_data, bufferlist& outdata)
{
...
bool ReplicatedPG::pgls_filter(PGLSFilter *filter, hobject_t& sobj, bufferlist& outdata)
{
  ...
  if (!filter->get_xattr().empty()) {
    ...
    if (ret < 0) {
      if (ret != -ENODATA || filter->reject_empty()) {
        return false;
      }
    }
  }

  return filter->filter(sobj.oid.name, bl, outdata);
}
